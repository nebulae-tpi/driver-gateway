...
process.env.GOOGLE_APPLICATION_CREDENTIALS = '/Users/sebastianmolano/NebulaE/Projects/TPI/driver-gateway/etc/gcloud-service-key.json';
...
describe('PUBSUB BROKER', function () {
    describe('Prepare PUBSUB broker', function () {
        it('instance PubSubBroker', function (done) {
            pubsubBroker = new PubSubBroker({
                ...
                gatewayRepliesTopic: 'driver-gateway-replies',
                gatewayRepliesTopicSubscription: 'driver-gateway-replies-test'
            });
    ...
    describe('Publish and listent on PubSub', function () {
        it('Publish and recive response using forward$ + getMessageReply$', function (done) {
            ...
            .pipe(
                switchMap(sentMessageId => forkJoin(
                    ...
                    .pipe(
                        delay(1000),
                        switchMap(() => pubsubBroker.forward$('driver-gateway-replies','Test', { x: 1, y: 2, z: 3 }, { correlationId: sentMessageId }) )
                    )
                ) )
            ).subscribe(
        ...
        //           .pipe(
        //              delay(200),
        //              switchMap(() => mqttBroker.forward$('driver-gateway-replies-test', { x: 1, y: 2, z: 3 }, { correlationId: messageId }))
        //            )
        //     ).subscribe(
        //         ([response, sentResponseMessageId]) => {
        //             assert.deepEqual(response, { x: 1, y: 2, z: 3 });
